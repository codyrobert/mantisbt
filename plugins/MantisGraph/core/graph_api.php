...
if( OFF == \Core\Plugin::config_get( 'eczlibrary' ) ) {
	$t_font_path = \Core\Utility::get_font_path();
...
function graph_get_font() {
	$t_font = \Core\Plugin::config_get( 'font', 'arial' );

	if( \Core\Plugin::config_get( 'eczlibrary' ) == ON ) {
		$t_font_map = array(
			'arial' => 'arial.ttf',
			'verdana' => 'verdana.ttf',
...
function graph_bar( array $p_metrics, $p_title = '', $p_graph_width = 350, $p_graph_height = 400 ) {
	...
	if( \Core\Plugin::config_get( 'eczlibrary' ) == ON ) {
		...
		$t_graph = new \Core\Graph( $p_graph_width, $p_graph_height );
		$t_graph->img->SetMargin( 40, 40, 40, 170 );
		if( ON == \Core\Plugin::config_get( 'jpgraph_antialias' ) ) {
		...
		$t_graph->Add( $t_plot1 );
		if( \Core\Helper::show_query_count() ) {
			$t_graph->subtitle->Set( \Core\Database::count_queries() . ' queries (' . \Core\Database::time_queries() . 'sec)' );
...
function graph_group( array $p_metrics, $p_title = '', $p_graph_width = 350, $p_graph_height = 400, $p_baseline = 100 ) {
	...
	if( \Core\Plugin::config_get( 'eczlibrary' ) == ON ) {
		...
		$t_tot->SetWidth( 0.7 );
		$t_tot->SetLegend( \Core\Plugin::langget( 'legend_total' ) );
		$t_graph->Add( $t_tot );

		$t_plot1 = new BarPlot( array_values( $p_metrics['open'] ) );
		$t_plot1->SetFillColor( 'yellow' );
		$t_plot1->SetWidth( 1 );
		$t_plot1->SetLegend( \Core\Plugin::langget( 'legend_opened' ) );

		$t_plot2 = new BarPlot( array_values( $p_metrics['closed'] ) );
		$t_plot2->SetFillColor( 'blue' );
		$t_plot2->SetWidth( 1 );
		$t_plot2->SetLegend( \Core\Plugin::langget( 'legend_closed' ) );

		$t_plot3 = new BarPlot( array_values( $p_metrics['resolved'] ) );
		$t_plot3->SetFillColor( 'red' );
		$t_plot3->SetWidth( 1 );
		$t_plot3->SetLegend( \Core\Plugin::langget( 'legend_resolved' ) );
...
function graph_cumulative_bydate( array $p_metrics, $p_graph_width = 300, $p_graph_height = 380 ) {
	$t_graph_font = graph_get_font();
	error_check( is_array( $p_metrics ) ? count( $p_metrics ) : 0, \Core\Plugin::langget( 'cumulative' ) . ' ' . \Core\Lang::get( 'by_date' ) );
	...
	if( \Core\Plugin::config_get( 'eczlibrary' ) == ON ) {
		...
		$t_graph->data[0]->label = \Core\Plugin::langget( 'legend_reported' );
		$t_graph->data[0]->color = '#FF0000';

		$t_graph->data[1] = new ezcGraphArrayDataSet( $p_metrics[1] );
		$t_graph->data[1]->label = \Core\Plugin::langget( 'legend_resolved' );
		$t_graph->data[1]->color = '#0000FF';

		$t_graph->data[2] = new ezcGraphArrayDataSet( $p_metrics[2] );
		$t_graph->data[2]->label = \Core\Plugin::langget( 'legend_still_open' );
		...
		$t_graph->title = \Core\Plugin::langget( 'cumulative' ) . ' ' . \Core\Lang::get( 'by_date' );
		$t_graph->options->font = $t_graph_font ;
		...
		$t_graph->SetMarginColor( 'white' );
		$t_graph->SetFrame( false );
		$t_graph->title->Set( \Core\Plugin::langget( 'cumulative' ) . ' ' . \Core\Lang::get( 'by_date' ) );
		$t_graph->title->SetFont( $t_graph_font, FS_BOLD );
		...
		$t_plot1->SetCenter();
		$t_plot1->SetLegend( \Core\Plugin::langget( 'legend_reported' ) );
		$t_graph->AddY2( $t_plot1 );

		$t_plot3 = new LinePlot( $t_still_open_plot, $t_plot_date );
		$t_plot3->SetColor( 'red' );
		$t_plot3->SetCenter();
		$t_plot3->SetLegend( \Core\Plugin::langget( 'legend_still_open' ) );
		$t_graph->Add( $t_plot3 );

		$t_plot2 = new LinePlot( $t_resolved_plot, $t_plot_date );
		$t_plot2->SetColor( 'black' );
		$t_plot2->SetCenter();
		$t_plot2->SetLegend( \Core\Plugin::langget( 'legend_resolved' ) );
...
function graph_bydate( array $p_metrics, array $p_labels, $p_title, $p_graph_width = 300, $p_graph_height = 380 ) {
	$t_graph_font = graph_get_font();
	error_check( is_array( $p_metrics ) ? count( $p_metrics ) : 0, \Core\Lang::get( 'by_date' ) );
	...
	if( \Core\Plugin::config_get( 'eczlibrary' ) == ON ) {
		...
		$t_graph->title = $p_title . ' ' . \Core\Lang::get( 'by_date' );
		...
		$t_graph->SetMarginColor( 'white' );
		$t_graph->SetFrame( false );
		$t_graph->title->Set( $p_title . ' ' . \Core\Lang::get( 'by_date' ) );
		$t_graph->title->SetFont( $t_graph_font, FS_BOLD );
		...
		# $t_line_colours = \Core\Plugin::config_get( 'jpgraph_colors' );
...
function create_bug_enum_summary( $p_enum_string, $p_enum ) {
	...
	$t_specific_where = ' AND ' . \Core\Helper::project_specific_where( $t_project_id, $t_user_id );

	$t_metrics = array();
	$t_assoc_array = \Core\Enum::getAssocArrayIndexedByValues( $p_enum_string );

	if( !\Core\Database::field_exists( $p_enum, \Core\Database::get_table( 'bug' ) ) ) {
		trigger_error( ERROR_DB_FIELD_NOT_FOUND, ERROR );
	}

	foreach ( $t_assoc_array as $t_value => $t_label ) {
		$t_query = 'SELECT COUNT(*) FROM {bug} WHERE ' . $p_enum . '=' . \Core\Database::param() . ' ' . $t_specific_where;
		$t_result = \Core\Database::query( $t_query, array( $t_value ) );
		$t_metrics[$t_label] = \Core\Database::result( $t_result, 0 );
	}
...
function enum_bug_group( $p_enum_string, $p_enum ) {
	$t_project_id = \Core\Helper::get_current_project();
	$t_user_id = \Core\Auth::get_current_user_id();
	$t_res_val = \Core\Config::mantis_get( 'bug_resolved_status_threshold' );
	$t_clo_val = \Core\Config::mantis_get( 'bug_closed_status_threshold' );
	$t_specific_where = ' AND ' . \Core\Helper::project_specific_where( $t_project_id, $t_user_id );
	...
	$t_array_indexed_by_enum_values = \Core\Enum::getAssocArrayIndexedByValues( $p_enum_string );
	foreach ( $t_array_indexed_by_enum_values as $t_value => $t_label ) {
		...
		$t_query = 'SELECT COUNT(*) FROM {bug}
					WHERE ' . $p_enum . '=' . \Core\Database::param() . ' AND
						status<' . \Core\Database::param() . ' ' . $t_specific_where;
		$t_result2 = \Core\Database::query( $t_query, array( $t_value, $t_res_val ) );
		$t_metrics['open'][$t_label] = \Core\Database::result( $t_result2, 0, 0 );

		# Calculates the number of bugs closed and puts the results in a table
		$t_query = 'SELECT COUNT(*) FROM {bug}
					WHERE ' . $p_enum . '=' . \Core\Database::param() . ' AND
						status>=' . \Core\Database::param() . ' ' . $t_specific_where;
		$t_result2 = \Core\Database::query( $t_query, array( $t_value, $t_clo_val ) );
		$t_metrics['closed'][$t_label] = \Core\Database::result( $t_result2, 0, 0 );
		...
		$t_query = 'SELECT COUNT(*) FROM {bug}
					WHERE ' . $p_enum . '=' . \Core\Database::param() . ' AND
						...
						status<' . \Core\Database::param() . ' ' . $t_specific_where;
		$t_result2 = \Core\Database::query( $t_query, array(  $t_value, $t_res_val, $t_clo_val ) );
		$t_metrics['resolved'][$t_label] = \Core\Database::result( $t_result2, 0, 0 );
	}
...
function create_developer_summary() {
	...
	\Core\User::cache_array_rows( $t_handlers );

	foreach( $t_handler_arr as $t_handler => $t_data ) {
		$t_username = \Core\User::get_name( $t_handler );
...
function create_reporter_summary() {
	...
	foreach( $t_reporter_arr as $t_reporter => $t_count ) {
		$t_metrics[\Core\User::get_name( $t_reporter )] = $t_count;
	}
	ksort( $t_metrics );
...
function create_category_summary() {
	...
	while( $t_row = \Core\Database::fetch_array( $t_result ) ) {
		...
		$t_cat_id = $t_row['id'];
		$t_query = 'SELECT COUNT(*) FROM {bug} WHERE category_id=' . \Core\Database::param() . ' AND ' . $t_specific_where;
		$t_result2 = \Core\Database::query( $t_query, array( $t_cat_id ) );
		if( isset($t_metrics[$t_cat_name]) ) {
			$t_metrics[$t_cat_name] = $t_metrics[$t_cat_name] + \Core\Database::result( $t_result2, 0, 0 );
		} else {
			if( \Core\Database::result( $t_result2, 0, 0 ) > 0 ) {
			    $t_metrics[$t_cat_name] = \Core\Database::result( $t_result2, 0, 0 );
			}
		}
	}
...
function create_cumulative_bydate() {
	...
	$t_query = 'SELECT {bug}.id, last_updated, date_modified, new_value, old_value
			...
			WHERE ' . $t_specific_where . '
						AND {bug}.status >= ' . \Core\Database::param() . '
						AND ( ( {bug_history}.new_value >= ' . \Core\Database::param() . '
								AND {bug_history}.field_name = \'status\' )
						OR {bug_history}.id is NULL )
			ORDER BY {bug}.id, date_modified ASC';
	$t_result = \Core\Database::query( $t_query, array( $t_res_val, (string)$t_res_val ) );
	$t_bug_count = \Core\Database::num_rows( $t_result );
...
function graph_date_format( $p_date ) {
	return date( \Core\Config::mantis_get( 'short_date_format' ), $p_date );
}
...
function error_check( $p_bug_count, $p_title ) {
	if( 0 == $p_bug_count ) {
		error_text( $p_title, \Core\Plugin::langget( 'not_enough_data' ) );
	}
}
