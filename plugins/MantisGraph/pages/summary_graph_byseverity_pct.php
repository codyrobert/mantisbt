...
if( $t_token == null ) {
	$t_metrics = create_bug_enum_summary( \Flickerbox\Lang::get( 'severity_enum_string' ), 'severity' );
} else {
	$t_metrics = graph_total_metrics( json_decode( $t_token, true ) );
}

graph_pie( $t_metrics, \Flickerbox\Plugin::langget( 'by_severity_pct' ), $f_width, $f_width );
