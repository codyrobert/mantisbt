...
if( $t_token == null ) {
	$t_metrics = create_bug_enum_summary( \Core\Lang::get( 'resolution_enum_string' ), 'resolution' );
} else {
	 $t_metrics = graph_total_metrics( json_decode( $t_token, true ) );
}

graph_pie( $t_metrics, \Core\Plugin::langget( 'by_resolution_pct' ), $f_width, $f_width );
