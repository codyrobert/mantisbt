...
function mc_enum_get( $p_username, $p_password, $p_enumeration ) {
	...
	return \Core\Lang::get( $p_enumeration . '_enum_string', $t_lang );
}
...
function mci_enum_get_array_by_id( $p_enum_id, $p_enum_type, $p_lang ) {
	...
	$t_result['name'] = mci_helper_get_enum_element( $p_enum_type, $p_enum_id, $p_lang );
	return $t_result;
}
...
function mci_get_enum_id_from_objectref( $p_enum, $p_object_ref ) {
	...
	if( !is_null( $p_object_ref ) && isset( $p_object_ref['id'] ) && (int)$p_object_ref['id'] != 0 ) {
		...
		$t_enum = \Core\Config::mantis_get( $p_enum . '_enum_string' );
		if( !is_null( $p_object_ref ) && isset( $p_object_ref['name'] ) && !\Core\Utility::is_blank( $p_object_ref['name'] ) ) {
			$t_id = mci_get_enum_value_from_label( $t_enum, $p_object_ref['name'] );
			if( $t_id == 0 ) {
				$t_id = \Core\Config::mantis_get( 'webservice_' . $p_enum . '_enum_default_when_not_found' );
			}
		} else {
			$t_default_id = \Core\Config::mantis_get( 'default_bug_' . $p_enum, 0 );
			if( $t_default_id == 0 ) {
