...
function mci_file_can_download_bug_attachments( $p_bug_id, $p_user_id ) {
	$t_can_download = \Core\Access::has_bug_level( \Core\Config::mantis_get( 'download_attachments_threshold' ), $p_bug_id );
	if( $t_can_download ) {
		return true;
	}

	$t_reported_by_me = \Core\Bug::is_user_reporter( $p_bug_id, $p_user_id );
	return( $t_reported_by_me && \Core\Config::mantis_get( 'allow_download_own_attachments' ) );
}
...
function mci_file_add( $p_id, $p_name, $p_content, $p_file_type, $p_table, $p_title = '', $p_desc = '', $p_user_id = null ) {
	if( !\Core\File::type_check( $p_name ) ) {
		return SoapObjectsFactory::newSoapFault( 'Client', 'File type not allowed.' );
	}
	if( !\Core\File::is_name_unique( $p_name, $p_id ) ) {
	...
	if( 'bug' == $p_table ) {
		$t_project_id = \Core\Bug::get_field( $p_id, 'project_id' );
		$t_id = (int)$p_id;
		$t_issue_id = \Core\Bug::format_id( $p_id );
	} else {
		$t_project_id = $p_id;
		$t_id = $t_project_id;
		$t_issue_id = 0;
	...
	switch( $t_method ) {
		case DISK:
			...
			break;
		case DATABASE:
			$c_content = \Core\Database::prepare_binary_string( $p_content );
			$t_file_path = '';
			break;
	}
	...
	\Core\Database::query( $t_query, array(
		...
		$t_file_size, $p_file_type, \Core\Database::now(),
		(int)$p_user_id,
	) );
	...
	if( 'bug' == $p_table ) {
		...
		\Core\Bug::update_date( $t_issue_id );

		# add history entry
		\Core\History::log_event_special( $t_issue_id, FILE_ADDED, $p_name );
	}
...
function mci_file_get( $p_file_id, $p_type, $p_user_id ) {
	...
	$t_diskfile = \Core\File::normalize_attachment_path( $t_row['diskfile'], $t_project_id );
	...
	switch( $p_type ) {
		...
		case 'doc':
			...
			if( !\Core\Access::has_project_level( \Core\Config::mantis_get( 'view_proj_doc_threshold' ), $t_project_id, $p_user_id ) ) {
				return mci_soap_fault_access_denied( $p_user_id );
			}
			break;
	}
