...
$f_days = \Core\GPC::get_int( 'days', 0 );
$f_all = \Core\GPC::get_int( 'all', 0 );

$t_end_time = time() - ( $f_days * SECONDS_PER_DAY );
$t_start_time = $t_end_time - ( 7 * SECONDS_PER_DAY );
$t_events = \Core\Timeline::events( $t_start_time, $t_end_time );

echo '<div class="timeline">';

$t_heading = \Core\Lang::get( 'timeline_title' );
...
$t_prev_link = ' [<a href="my_view_page.php?days=' . ( $f_days + 7 ) . '">' . \Core\Lang::get( 'prev' ) . '</a>]';

if( $t_next_days != $f_days ) {
	$t_next_link = ' [<a href="my_view_page.php?days=' . $t_next_days . '">' . \Core\Lang::get( 'next' ) . '</a>]';
} else {
	$t_next_link = '';
}
...
$t_events = \Core\Timeline::sort_events( $t_events );

$t_num_events = \Core\Timeline::print_events( $t_events, ( $f_all ? 0 : MAX_EVENTS ) );
...
if( !$f_all && $t_num_events < count( $t_events )) {
	echo '<p>' . $t_prev_link = ' [ <a href="my_view_page.php?days=' . $f_days . '&amp;all=1">' . \Core\Lang::get( 'timeline_more' ) . '</a> ]</p>';
}
