...
class IssueAssignedTimelineEvent extends TimelineEvent {
	...
	public function html() {
		if( $this->user_id == $this->handler_id ) {
			$t_string = sprintf( \Flickerbox\Lang::get( 'timeline_issue_assigned_to_self' ), user_get_name( $this->user_id ), \Flickerbox\String::get_bug_view_link( $this->issue_id ) );
		} else if( $this->handler_id != NO_USER ) {
			$t_string = sprintf( \Flickerbox\Lang::get( 'timeline_issue_assigned' ), user_get_name( $this->user_id ), \Flickerbox\String::get_bug_view_link( $this->issue_id ), user_get_name( $this->handler_id ) );
		} else {
			$t_string = sprintf( \Flickerbox\Lang::get( 'timeline_issue_unassigned' ), user_get_name( $this->user_id ), \Flickerbox\String::get_bug_view_link( $this->issue_id ) );
		}
