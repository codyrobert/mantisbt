...
class MantisEnum {
	...
	public static function getLabel( $p_enum_string, $p_value ) {
		...
		return \Flickerbox\MantisEnum::getLabelForUnknownValue( $t_value_as_integer );
	}
	...
	public static function getLocalizedLabel( $p_enum_string, $p_localized_enum_string, $p_value ) {
		if( !\Flickerbox\MantisEnum::hasValue( $p_enum_string, $p_value ) ) {
			return \Flickerbox\MantisEnum::getLabelForUnknownValue( $p_value );
		}

		$t_assoc_array = \Flickerbox\MantisEnum::getAssocArrayIndexedByValues( $p_localized_enum_string );

		if( isset( $t_assoc_array[(int)$p_value] ) ) {
			return $t_assoc_array[(int)$p_value];
		}

		return \Flickerbox\MantisEnum::getLabel( $p_enum_string, $p_value );
	}
	...
	public static function getValue( $p_enum_string, $p_label ) {
		$t_assoc_array_by_labels = \Flickerbox\MantisEnum::getAssocArrayIndexedByLabels( $p_enum_string );
	...
	public static function getAssocArrayIndexedByValues( $p_enum_string ) {
		...
		$t_tuples = \Flickerbox\MantisEnum::getArrayOfTuples( $p_enum_string );

		$t_assoc_array = array();

		foreach ( $t_tuples as $t_tuple ) {
			$t_tuple_tokens = \Flickerbox\MantisEnum::getArrayForTuple( $t_tuple );
	...
	public static function getAssocArrayIndexedByLabels( $p_enum_string ) {
		return array_flip( \Flickerbox\MantisEnum::getAssocArrayIndexedByValues( $p_enum_string ) );
	}
	...
	public static function getValues( $p_enum_string ) {
		return array_unique( array_keys( \Flickerbox\MantisEnum::getAssocArrayIndexedByValues( $p_enum_string ) ) );
	}
	...
	private static function getArrayOfTuples( $p_enum_string ) {
		...
		$t_raw_array = explode( \Flickerbox\MantisEnum::TUPLE_SEPARATOR, $p_enum_string );
	...
	private static function getArrayForTuple( $p_tuple ) {
		return explode( \Flickerbox\MantisEnum::VALUE_LABEL_SEPARATOR, $p_tuple );
	}
