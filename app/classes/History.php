...
class History
{
	...
	static function get_events_array( $p_bug_id, $p_user_id = null ) {
		...
		foreach( $t_raw_history as $k => $t_item ) {
			...
			$t_history[$k] = \Flickerbox\History::localize_item( $v_field, $v_type, $v_old_value, $v_new_value );
	...
	static function localize_item( $p_field_name, $p_type, $p_old_value, $p_new_value, $p_linkify = true ) {
		...
		switch( $p_field_name ) {
			case 'category':
				...
				break;
			case 'status':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'status', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'status', $p_new_value );
				$t_field_localized = \Flickerbox\Lang::get( 'status' );
				break;
			case 'severity':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'severity', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'severity', $p_new_value );
				$t_field_localized = \Flickerbox\Lang::get( 'severity' );
				break;
			case 'reproducibility':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'reproducibility', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'reproducibility', $p_new_value );
				$t_field_localized = \Flickerbox\Lang::get( 'reproducibility' );
				break;
			case 'resolution':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'resolution', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'resolution', $p_new_value );
				$t_field_localized = \Flickerbox\Lang::get( 'resolution' );
				break;
			case 'priority':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'priority', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'priority', $p_new_value );
				$t_field_localized = \Flickerbox\Lang::get( 'priority' );
				break;
			case 'eta':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'eta', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'eta', $p_new_value );
				$t_field_localized = \Flickerbox\Lang::get( 'eta' );
				break;
			case 'view_state':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'view_state', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'view_state', $p_new_value );
				$t_field_localized = \Flickerbox\Lang::get( 'view_status' );
				break;
			case 'projection':
				$p_old_value = \Flickerbox\Helper::get_enum_element( 'projection', $p_old_value );
				$p_new_value = \Flickerbox\Helper::get_enum_element( 'projection', $p_new_value );
			...
			case 'sticky':
				...
				break;
			case 'project_id':
				if( \Flickerbox\Project::exists( $p_old_value ) ) {
					$p_old_value = \Flickerbox\Project::get_field( $p_old_value, 'name' );
				} else {
				...
				# current one.
				if( \Flickerbox\Project::exists( $p_new_value ) ) {
					$p_new_value = \Flickerbox\Project::get_field( $p_new_value, 'name' );
				} else {
