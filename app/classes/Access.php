...
class Access
{
	...
	static function get_global_level( $p_user_id = null ) {
		...
		if( empty( $p_user_id ) && !\Flickerbox\Auth::is_user_authenticated() ) {
			return false;
		}
	...
	static function has_global_level( $p_access_level, $p_user_id = null ) {
		...
		$t_access_level = \Flickerbox\Access::get_global_level( $p_user_id );
	
		return \Flickerbox\Access::compare_level( $t_access_level, $p_access_level );
	}
	...
	static function ensure_global_level( $p_access_level, $p_user_id = null ) {
		if( !\Flickerbox\Access::has_global_level( $p_access_level, $p_user_id ) ) {
	...
	static function ensure_project_level( $p_access_level, $p_project_id = null, $p_user_id = null ) {
		if( !\Flickerbox\Access::has_project_level( $p_access_level, $p_project_id, $p_user_id ) ) {
			\Flickerbox\Access::denied();
		}
	}
	...
	static function has_bug_level( $p_access_level, $p_bug_id, $p_user_id = null ) {
		...
		if( $t_limit_reporters && !$t_bug_is_user_reporter ) {
			...
			if( !\Flickerbox\Access::compare_level( $t_access_level, $s_thresholds[$t_project_id] ) ) {
				return false;
			}
		}
	...
	static function ensure_bug_level( $p_access_level, $p_bug_id, $p_user_id = null ) {
		if( !\Flickerbox\Access::has_bug_level( $p_access_level, $p_bug_id, $p_user_id ) ) {
			\Flickerbox\Access::denied();
		}
	}
	...
	static function has_bugnote_level( $p_access_level, $p_bugnote_id, $p_user_id = null ) {
		...
		return \Flickerbox\Access::has_bug_level( $p_access_level, $t_bug_id, $p_user_id );
	}
	...
	static function ensure_bugnote_level( $p_access_level, $p_bugnote_id, $p_user_id = null ) {
		if( !\Flickerbox\Access::has_bugnote_level( $p_access_level, $p_bugnote_id, $p_user_id ) ) {
			\Flickerbox\Access::denied();
		}
	}
	...
	static function can_reopen_bug( \Flickerbox\BugData $p_bug, $p_user_id = null ) {
		...
		if( \Flickerbox\Access::has_bug_level( $t_reopen_bug_threshold, $p_bug->id, $p_user_id ) ) {
			...
			return \Flickerbox\Access::has_bug_level( $t_reopen_status_threshold, $p_bug->id, $p_user_id );
		}
	...
	static function get_local_level( $p_user_id, $p_project_id ) {
		...
		$t_project_level = \Flickerbox\Access::cache_matrix_user( $p_user_id );
