...
class Lang
{
	...
	static function load( $p_lang, $p_dir = null ) {
		...
		if( !\Flickerbox\Lang::language_exists( $p_lang ) ) {
			return;
		}
	...
	static function get_default() {
		...
		if( $t_lang == 'auto' ) {
			$t_lang = \Flickerbox\Lang::map_auto();
		}
	...
	*/
	...
	static function push( $p_lang = null ) {
		...
		\Flickerbox\Lang::ensure_loaded( $t_lang );
	}
	...
	static function get_current() {
		...
		if( $t_count_overrides > 0 ) {
			...
			$t_lang = \Flickerbox\Lang::get_default();
		}
	...
	static function get( $p_string, $p_lang = null ) {
		...
		if( \Flickerbox\Lang::exists( $p_string, $t_lang ) ) {
			...
			if( $t_lang == 'english' ) {
				\Flickerbox\Error::parameters( $p_string );
				...
				return \Flickerbox\Lang::get( $p_string, 'english' );
			}
		}
	}
	...
	/**
	 * Get language:
	 * - If found, return the appropriate string (as \Flickerbox\Lang::get()).
	...
	static function get_defaulted( $p_string, $p_default = null, $p_lang = null ) {
		...
		if( \Flickerbox\Lang::exists( $p_string, $t_lang ) ) {
			return \Flickerbox\Lang::get( $p_string );
		} else {
			if( null === $p_default ) {
				return $p_string;
			} else {
