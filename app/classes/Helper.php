...
class Helper
{
	 ...
	 * @return string
	 */
	static function alternate_colors( $p_index, $p_odd_color, $p_even_color ) {
		static $s_index = 1;
	 ...
	 * @return string
	 */
	static function alternate_class( $p_index = null, $p_odd_class = 'row-1', $p_even_class = 'row-2' ) {
		static $t_index = 1;
	 ...
	 */
	static function array_transpose( array $p_array ) {
		if( !is_array( $p_array ) ) {
			return $p_array;
		}
		$t_out = array();
	 ...
	 * @return boolean
	 */
	static function check_variables_equal( $p_var1, $p_var2, $p_strict ) {
		if( $p_strict ) {
	 ...
	 * @return void
	 */
	static function check_disabled( $p_val = true ) {
		if( $p_val ) {
	 ...
	 * @return integer
	 */
	static function get_current_project() {
		...
		if( $g_cache_current_project === null ) {
			...
			if( !\Flickerbox\Project::exists( $t_project_id ) || ( 0 == \Flickerbox\Project::get_field( $t_project_id, 'enabled' ) ) || !\Flickerbox\Access::has_project_level( config_get( 'view_bug_threshold', null, null, $t_project_id ), $t_project_id ) ) {
	...
	static function get_current_project_trace() {
		...
		if( !\Flickerbox\Project::exists( $t_bottom ) || ( 0 == \Flickerbox\Project::get_field( $t_bottom, 'enabled' ) ) || !\Flickerbox\Access::has_project_level( config_get( 'view_bug_threshold', null, null, $t_bottom ), $t_bottom ) ) {
			$t_project_id = array(
				ALL_PROJECTS,
			);
		}
	 ...
	 * @return mixed
	 */
	static function call_custom_function( $p_function, array $p_args_array ) {
	 ...
	 * @return integer
	 */
	static function get_tab_index_value() {
